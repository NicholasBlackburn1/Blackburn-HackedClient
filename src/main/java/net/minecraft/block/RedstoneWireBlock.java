...
public class RedstoneWireBlock extends Block {
   ...
   private RedstoneSide getSide(IBlockReader worldIn, BlockPos pos, Direction face) {
      ...
      return !canConnectTo(blockstate, face) && (blockstate.isNormalCube(worldIn, blockpos) || !canConnectUpwardsTo(worldIn.getBlockState(blockpos.down()))) ? RedstoneSide.NONE : RedstoneSide.SIDE;
   }
   ...
   private boolean isPowerSourceAt(IBlockReader worldIn, BlockPos pos, Direction side) {
      ...
      if (!flag1 && flag && canConnectUpwardsTo(worldIn, blockpos.up())) {
         return true;
      } else if (canConnectTo(blockstate, side)) {
         return true;
