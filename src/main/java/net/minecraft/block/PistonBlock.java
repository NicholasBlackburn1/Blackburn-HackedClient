   ...
   public boolean eventReceived(BlockState state, World worldIn, BlockPos pos, int id, int param) {
      ...
      if (id == 0) {
         ...
         if (this.isSticky) {
            ...
            if (!flag1) {
               if (id != 1 || blockstate.isAir() || !canPush(blockstate, worldIn, blockpos, direction.getOpposite(), false, direction) || blockstate.getPushReaction() != PushReaction.NORMAL && block != Blocks.PISTON && block != Blocks.STICKY_PISTON) {
   ...
   private boolean doMove(World worldIn, BlockPos pos, Direction directionIn, boolean extending) {
      ...
      if (!pistonblockstructurehelper.canMove()) {
         ...
         for(int j = list2.size() - 1; j >= 0; --j) {
            ...
            TileEntity tileentity = blockstate.getBlock().hasTileEntity() ? worldIn.getTileEntity(blockpos2) : null;
