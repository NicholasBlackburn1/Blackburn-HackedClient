      ...
      if (worldIn.getGameRules().getBoolean(GameRules.DO_FIRE_TICK)) {
         ...
         if (!flag && worldIn.isRaining() && this.canDie(worldIn, pos) && random.nextFloat() < 0.2F + (float)i * 0.03F) {
            ...
            if (!flag) {
               ...
               if (i == 15 && random.nextInt(4) == 0 && !this.canBurn(worldIn.getBlockState(pos.down()))) {
                  worldIn.removeBlock(pos, false);
                  return;
               }
            }
            ...
            int k = flag1 ? -50 : 0;
            this.catchOnFire(worldIn, pos.east(), 300 + k, random, i);
            this.catchOnFire(worldIn, pos.west(), 300 + k, random, i);
            this.catchOnFire(worldIn, pos.down(), 250 + k, random, i);
            this.catchOnFire(worldIn, pos.up(), 250 + k, random, i);
            this.catchOnFire(worldIn, pos.north(), 300 + k, random, i);
            this.catchOnFire(worldIn, pos.south(), 300 + k, random, i);
