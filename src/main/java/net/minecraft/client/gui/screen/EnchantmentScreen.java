...
public class EnchantmentScreen extends ContainerScreen<EnchantmentContainer> {
   ...
   public void render(int p_render_1_, int p_render_2_, float p_render_3_) {
      ...
      for(int j = 0; j < 3; ++j) {
         ...
         int i1 = j + 1;
         if (this.isPointInRegion(60, 14 + 19 * j, 108, 17, (double)p_render_1_, (double)p_render_2_) && k > 0 && l >= 0 && enchantment != null) {
            List<String> list = Lists.newArrayList();
            list.add("" + TextFormatting.WHITE + TextFormatting.ITALIC + I18n.format("container.enchant.clue", enchantment.getDisplayName(l).getFormattedText()));
            if (!flag) {
               list.add("");
